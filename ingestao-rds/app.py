...
def load_data(table_name, coins_df, session_db, engine_db):
    ...
    session_db.commit()
    session_db.close() # finaliza a sessão
...
def get_data(session_db, engine_db, start, limit, convert, key, url):
    ...
    session.headers.update(headers) # instancia os headers
    ...
    try:
        response = session.get(url, params=parameters) # obtem os dados da API
        data = json.loads(response.text) # guardar na variável 'data' em formato json

        print ('\n') # busca as informações específicas nos dados gerados
